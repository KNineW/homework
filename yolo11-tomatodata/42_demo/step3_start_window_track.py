...
WELCOME_SENTENCE = "欢迎使用农作物成熟度检测系统"   # 欢迎的句子
ICON_IMAGE = "images/UI/logo.png"                 # 系统logo界面
IMAGE_LEFT_INIT = "images/UI/up.jpeg"              # 图片检测界面初始化左侧图像
IMAGE_RIGHT_INIT = "images/UI/right.jpeg"          # 图片检测界面初始化右侧图像
ZHU_IMAGE_PATH = "images/UI/zhu.jpeg"
USERNAME = "123"                             # 用户名
PASSWORD = "123"                             # 密码
LOGIN_TITLE = "欢迎使用基于YOLO系列的识别系统"
...
class MainWindow(QTabWidget):
    ...
    def initUI(self):
        ...
        config_icon_label.setPixmap(QPixmap("images/UI/config.jpeg"))
        ...
        while self.cap.isOpened():
            ...
            if success:
                ...
                if self.config_track_value.currentText() == "不开启追踪":
                    ...
                    for idx_cls, cls_num in enumerate(result_nums):
                        if cls_num > 0:
                            result_info = result_info + "{}:{}".format(result_names[idx_cls], cls_num)
                        # result_info = result_info + "{}:{}\n".format(result_names[idx_cls], cls_num)
                        # result_info = result_info + "{}:{}\n".format(result_names[idx_cls], cls_num)
